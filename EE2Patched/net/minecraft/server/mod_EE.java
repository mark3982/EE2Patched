...
public class mod_EE extends NetworkMod
{
    ...
    if (!EEBase.externalModsInitialized)
    {
      for (int var4 = 0; var4 < ModLoader.getLoadedMods().size(); var4++)
      {
        ...
        else if (((BaseMod)ModLoader.getLoadedMods().get(var4)).toString().contains("mod_BuildCraftEnergy"))
        {
          ...
          if (!((BaseMod)ModLoader.getLoadedMods().get(var4)).toString().contains("mod_Forestry"))
            continue;
    ...
    while (var3.hasNext())
    {
      ...
      for (int var11 = 0; var11 < var10; var11++)
      {
        ...
        else if ((var12.getItem() == EEItem.volcanite) && (EEBase.isPlayerInLava(var4)))
        {
          ...
          if (var12.getItem() != EEItem.swiftWolfRing)
            continue;
          var8 = true;
          var4.abilities.canFly = true;
        }
  ...
  private void doFlightCheck(List var1, World var2)
  {
    ...
    while (var3.hasNext())
    {
	  ...
	  inv = var4.inventory.items;

	  for (x = 0; x < 9; ++x) {
		  if (inv[x] == null)
			continue;
		  if ((inv[x].getItem() == EEItem.arcaneRing) ||
			  (inv[x].getItem() == EEItem.evertide) ||
			  (inv[x].getItem() == EEItem.volcanite) ||
			  (inv[x].getItem() == EEItem.swiftWolfRing)
		  ) {
			  if (inv[x].getItem() == EEItem.swiftWolfRing) {
				  ItemStack			ks;

				  // MAKE SURE IT HAS SOME CHARGE OR MOVE IT OUT
				  // OF THE QUICK BAR THIS IS TO PREVENT SOMEONE
				  // FROM SPAMMING THE SPACE BAR AND HOVERING AROUND
				  // UNLESS THEY ACTUALLY HAVE A HACKED CLIENT..
				  ks = ItemToolHigh.getPlayerKleinStar(var4);
				  emsg = "You are missing a klein in your inventory.";
				  if (ks != null) {
					  if (ItemToolHigh.hasKleinCharge(ks, 8192 / 30 / 10)) {
						  continue;
					  }
					  emsg = "No klein in your inventory has enough charge to equip the ring.";
				  }
			  } else {
				  emsg = "Flying For This Ring Has Been Disabled. Use Swiftwolf's Rendering Gale.";
			  }
			  
			  for (y = 9; y < inv.length; ++y) {
				  if (inv[y] == null)
					break;
				  if ((inv[y].getItem() != EEItem.arcaneRing) &&
					  (inv[y].getItem() != EEItem.evertide) &&
					  (inv[y].getItem() != EEItem.volcanite) &&
					  (inv[y].getItem() != EEItem.swiftWolfRing)
				  ) {
					break;
				}
			  }
			  if (y == inv.length) {
				Bukkit.getServer().getPlayer(var4.name).sendMessage("Oh.. You think your fucking smart huh? Well, say bye to the ring now bitch.");
				inv[x] = null;
			  } else {
				tmpswap = inv[y];
				inv[y] = inv[x];
			  ...
			  Bukkit.getServer().getPlayer(var4.name).sendMessage(emsg);
		  }
      }
      ///////////////////
      boolean var5 = false;
